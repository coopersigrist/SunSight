...
state_df['realized_potential_percent'] = state_df['panel_utilization'].values * 100
state_df['payback_period'] = state_df['Adjusted Payback Period (Years, under energy generation assumptions)']
...
# plot_state_map(state_df[state_df['State'] != 'Hawaii'], key='realized_potential_percent', fill_color='Blues', legend_name="Realized Potential", save_dir_prefix="../Visualization/", show=False)
plot_state_map(state_df, key='payback_period', fill_color='Blues', legend_name="Adjusted Payback Period", save_dir_prefix="../Visualization/", show=False)
