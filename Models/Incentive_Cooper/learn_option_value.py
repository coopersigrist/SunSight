...
def option_val_multiplier_per_state(panel_size_watts=6637.375):
    ...
    for state, group in census_by_state:
        ...
        small_df.to_csv(state[0]+'_small_results_9.csv')
        ...
        #plt.show()
        df = pd.concat([df, new_row], ignore_index=True)
        df.to_csv("output_9.csv", index=False, header=new_row.columns)
...
def generate_agents_for_zips():
    df_distribution = pd.DataFrame(columns=['state_abbr', 'zip' 'energy_expenditures', 'incomes'])
    df_distributions = pd.DataFrame(columns=['state_abbr', 'zip', 'npv_vals'])
def generate_state_estimated_option_values_no_agents(panel_size_watts=6637.375):
...
if __name__ == "__main__":
    ...
    generate_state_estimated_option_values_no_agents(panel_size_watts=6637.375)
